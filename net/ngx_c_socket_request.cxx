...
ssize_t CSocekt::recvproc(lpngx_connection_t pConn,char *buff,ssize_t buflen)  //ssize_t是有符号整型，在32位机器上等同与int，在64位机器上等同与long int，size_t就是无符号型的ssize_t
{
    ...
    if(n < 0) //这被认为有错误发生
    {
        ...
        // EINTR是Linux系统中的一个错误码，意思是操作被信号中断。在进行一些系统调用（如read、write等）时，如果进程接收到了某个信号，那么这些系统调用就会立即返回并且设置errno为EINTR。
        // 这种情况通常发生在进程使用阻塞式I/O时，当进程接收到信号时，该I/O操作将被中断。处理该错误通常需要重新执行被中断的操作。
